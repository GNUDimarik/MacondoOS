...
TEST(LlvmLibcStrCaseCmpTest, EmptyStringsShouldReturnZero) {
    const char *s1 = "";
    const char *s2 = "";
    int result = __STD_NAMESPACE::strcasecmp(s1, s2);
    ASSERT_EQ(result, 0);

    // Verify operands reversed.
    result = __STD_NAMESPACE::strcasecmp(s2, s1);
    ASSERT_EQ(result, 0);
}
...
TEST(LlvmLibcStrCaseCmpTest, EmptyStringShouldNotEqualNonEmptyString) {
    const char *empty = "";
    const char *s2 = "abc";
    int result = __STD_NAMESPACE::strcasecmp(empty, s2);
    // This should be '\0' - 'a' = -97
    ASSERT_EQ(result, -97);

    // Similar case if empty string is second argument.
    const char *s3 = "123";
    result = __STD_NAMESPACE::strcasecmp(s3, empty);
...
TEST(LlvmLibcStrCaseCmpTest, StringArgumentSwapChangesSign) {
    const char *a = "a";
    const char *b = "B";
    int result = __STD_NAMESPACE::strcasecmp(b, a);
    // 'b' - 'a' = 1.
    ASSERT_EQ(result, 1);

    result = __STD_NAMESPACE::strcasecmp(a, b);
    // 'a' - 'b' = -1.
    ASSERT_EQ(result, -1);
}