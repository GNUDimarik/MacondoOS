...
TEST(LlvmLibcstrncasecmpTest, EmptyStringsShouldReturnZeroWithSufficientLength) {
    ...
    result = __STD_NAMESPACE::strncasecmp(s2, s1, 1);
    ASSERT_EQ(result, 0);
}
...
TEST(LlvmLibcstrncasecmpTest,
    ...
    const char *empty = "";
    const char *s2 = "abc";
    int result = __STD_NAMESPACE::strncasecmp(empty, s2, 3);
    // This should be '\0' - 'a' = -97
    ASSERT_EQ(result, -97);

    // Similar case if empty string is second argument.
    const char *s3 = "123";
    result = __STD_NAMESPACE::strncasecmp(s3, empty, 3);
...
TEST(LlvmLibcstrncasecmpTest, EqualStringsShouldReturnZeroWithSufficientLength) {
    ...
    // Verify operands reversed.
    result = __STD_NAMESPACE::strncasecmp(s2, s1, 3);
    ASSERT_EQ(result, 0);
}
...
TEST(LlvmLibcstrncasecmpTest,
    ...
    int result = __STD_NAMESPACE::strncasecmp(s1, s2, 8);
    // This should return 'B' - 'C' = -1.
    ASSERT_EQ(result, -1);

    // Verify operands reversed.
    result = __STD_NAMESPACE::strncasecmp(s2, s1, 8);
...
TEST(LlvmLibcstrncasecmpTest,
    ...
    const char *s1 = "AbCd";
    const char *s2 = "aBcD";
    int result = __STD_NAMESPACE::strncasecmp(s1, s2, 4);
    ASSERT_EQ(result, 0);

    // Verify operands reversed.
    result = __STD_NAMESPACE::strncasecmp(s2, s1, 4);
    ASSERT_EQ(result, 0);
}
