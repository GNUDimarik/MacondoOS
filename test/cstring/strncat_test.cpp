...
TEST(LlvmLibcStrNCatTest, EmptyDest) {
  ...
  char *result = __MACONDO_TEST_NAMESPACE::strncat(dest, abc, 0);
  ASSERT_EQ(dest, result);
  ASSERT_EQ(dest[0], '\0');

  // Then copy part of it.
  result = __MACONDO_TEST_NAMESPACE::strncat(dest, abc, 1);
  ASSERT_EQ(dest, result);
  ASSERT_STREQ(dest, "a");
  ...
  // Then copy all of it.
  result = __MACONDO_TEST_NAMESPACE::strncat(dest, abc, 3);
  ASSERT_EQ(dest, result);
...
TEST(LlvmLibcStrNCatTest, NonEmptyDest) {
  ...
  char *result = __MACONDO_TEST_NAMESPACE::strncat(dest, abc, 1);
  ASSERT_EQ(dest, result);
  ASSERT_STREQ(dest, "xyza");

  // Copy a bit more, but without resetting.
  result = __MACONDO_TEST_NAMESPACE::strncat(dest, abc, 2);
  ASSERT_EQ(dest, result);
  ...
  dest[4] = 'Z';
  result = __MACONDO_TEST_NAMESPACE::strncat(dest, abc, 4);
  ASSERT_EQ(dest, result);
  ASSERT_STREQ(dest, "abc");
  ASSERT_EQ(dest[4], 'Z');

  result = __MACONDO_TEST_NAMESPACE::strncat(dest, abc, 5);
  ASSERT_EQ(dest, result);
