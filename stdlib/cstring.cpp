...
using namespace __STD_NAMESPACE;
...
void *memchr(const void *memory, int needle, size_t count)
{
    ...
    while (count-- > 0) {
        if (*p++ == static_cast<unsigned char> (needle)) {
            return --p;
        }
    }
...
{
    return __STD_NAMESPACE::memcmp(first, second, len);
}
...
{
    return reinterpret_cast<char *> (__STD_NAMESPACE::memcpy(dest, src, n));
}
...
size_t strspn(const char *str, const char *accept)
{
    ...
    while (*str) {
        ...
        if (__STD_NAMESPACE::strchr(accept, *str++) == nullptr) {
            break;
        }
 ...
 *                  nullptr if not found no return value is reserved to indicate an
 *                  error.
 */
size_t strcspn(const char *str, const char *reject)
{
    ...
    while (*s) {
        if (__STD_NAMESPACE::strchr(reject, *s++) == nullptr) {
            res++;
        }
        else {
            break;
...
char *strpbrk(const char *str, const char *accept)
{
    ...
    while (*s) {
        if (__STD_NAMESPACE::strchr(accept, *s++) != nullptr) {
            return --s;
        }
    }
...
{
    size_t needle_len = __STD_NAMESPACE::strlen(needle);
    size_t haystack_len = __STD_NAMESPACE::strlen(haystack);
    size_t i = 0;
    ...
    for (i = haystack_len; i >= needle_len; i--, haystack++) {
        if (__STD_NAMESPACE::memcmp(haystack, needle, needle_len) == 0) {
            return (char *) haystack;
        }
    }
...
char *strtok_r(char *str, const char *delim, char **lasts)
{
    ...
    str += __STD_NAMESPACE::strspn(str, delim);
    ...
    res = str;
    str = __STD_NAMESPACE::strpbrk(res, delim);
